...
class MySQLDBLEObject extends DBLEObject{
    ...
    protected function generatePKConstructor() {
        ...
        $con2 = 'public function __construct2($dble,$pk) {'."\n".'
                $this->dble = $dble;'."\n".'
                $this->'.$pkvarname.' = $pk;'."\n".'
                $rez = $dble->select("SELECT * FROM '.$this->table.' WHERE '.$pkvarname.'=? LIMIT 1",array($pk));'."\n";
        $con2 .= 'if (!empty($rez)) {' . "\n";  
    ...
    protected function generateSaveToDB() {
        ...
        $sdb .= "} else {\n";
        $sdb .= '$this->'.$this->pk_column.' = $this->dble->insert("'.$sql2.'",'.$arr2.')' . ";\n";
        $sdb .= "}\n";
        $sdb .= "}\n";
        $this->code .= "\n\n" . $sdb . "\n\n";
    }
    ...
    protected function doesColumnExist($col) {
        $cols = $this->dble->getColumns($this->table);
        return in_array($col, $cols);
    }
    
    protected function isColumnPK($col) {
        $rez = $this->dble->select("SHOW INDEXES FROM ".$this->table." WHERE Key_name = 'PRIMARY'");
        return $col == $rez[0]["Column_name"];
    }
